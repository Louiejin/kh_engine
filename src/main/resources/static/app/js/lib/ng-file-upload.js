...
(function () {
  ...
  ngFileUpload.directive('ngfSrc', ['$compile', '$timeout', '$http', 'Upload', function ($compile, $timeout, $http, Upload) {
      return {
          restrict: 'AE',
              ...
              $timeout(function () {
                  var unwatch = scope.$watch(attr.ngfSrc, function (file) {
                      if (angular.isString(file)) {
                          ...
                          $http.get(file).then(function(){
                              elem.attr('src', file);
                          }, function(){
                              elem.attr('src', '/app/img/default_user.png');
                          });
                      } else if (file && file.type && file.type.indexOf(getTagType(elem[0])) === 0) {
